...
class Karyawan extends MY_Controller{
    ...
    function excel() {
        ...
        $object->getActiveSheet()->getStyle("A7:R7")->applyFromArray($style);
        $object->getActiveSheet()->getStyle("A7:R7")->applyFromArray($font);
        $object->getActiveSheet()->getStyle("A1:A5")->applyFromArray($font);
        $object->getActiveSheet()->getStyle('A7:R7')->getAlignment()->setWrapText(true);
        ...
        $object->getActiveSheet()->mergeCells('A5:R5');
        ...
        $object->setActiveSheetIndex(0)
            ...
            ->setCellValue('C7', 'Jenis Kelamin')
            ->setCellValue('D7', 'Nama Jabatan')
            ->setCellValue('E7', 'Pendidikan')
            ->setCellValue('F7', 'Tempat, Tanggal Lahir')
            ->setCellValue('G7', 'Agama')
            ->setCellValue('H7', 'Status')
            ->setCellValue('I7', 'Alamat')
            ->setCellValue('J7', 'Kelurahan')
            ->setCellValue('K7', 'Kecamatan')
            ->setCellValue('L7', 'Kota/Kabupaten')
            ->setCellValue('M7', 'BPJS Tenaga Kerja')
            ->setCellValue('N7', 'BPJS Kesehatan')
            ->setCellValue('O7', 'No Rekening')
            ->setCellValue('P7', 'No Handphone')
            ->setCellValue('Q7', 'Nama PJTK')
            ->setCellValue('R7', 'TMT Kontrak')
        ;
        $no=0;
        //add data
        $counter=8;
        ...
        foreach($result as $row){
            ...
            $object->getActiveSheet()->getStyle("P".$counter)->applyFromArray($style);
            $object->getActiveSheet()->getStyle("Q".$counter)->applyFromArray($style);
            $object->getActiveSheet()->getStyle("R".$counter)->applyFromArray($style);

            $ex->setCellValue("A".$counter,"$no");
            $ex->setCellValue("B".$counter,"$row->nama");
            $ex->setCellValue("C".$counter,"$row->jenis_kelamin");
            $ex->setCellValue("D".$counter,"$row->jabatan");
            $ex->setCellValue("E".$counter,"$row->jurusan");
            $ex->setCellValue("F".$counter,"$row->tmpt_lahir, $row->tgl_lahir");
            $ex->setCellValue("G".$counter,"$row->agama");
            $ex->setCellValue("H".$counter,"$row->status/$row->jumlah_anak");
            $ex->setCellValue("I".$counter,"$row->alamat");
            $ex->setCellValue("J".$counter,"$row->kelurahan");
            $ex->setCellValue("K".$counter,"$row->kecamatan");
            $ex->setCellValue("L".$counter,"$row->kota");
            $ex->setCellValue("M".$counter,"$row->bpjs_tenagakerja");
            $ex->setCellValue("N".$counter,"$row->bpjs_kesehatan");
            $ex->setCellValue("O".$counter,"$row->no_rekening");
            $ex->setCellValue("P".$counter,"$row->no_hp");
            $ex->setCellValue("Q".$counter,"$row->pjtk");
            $ex->setCellValue("R".$counter,"$row->tmt_kontrak");
            $counter=$counter+1;
        }
        ...
        header('Content-Disposition: attachment;filename="Rekap_Data_Karyawan_OC_PT_KKT.xlsx"');
