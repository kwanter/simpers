...
class Permohonan extends CI_Controller {
		  ...
		  foreach($wrp->result() as $r) {
				...
				*/
				...
				if ($r->num_state==1){
					...
					$action .="<a onclick='filldok(this.id)' class='btn btn-success btn-xs' title='Submit Registrasi' id=". $r->id_permohonan."><i class='fa fa-check-square-o'></i></a>&nbsp;";
	...
	public function ajax_list2()
	{
		  ...
		  foreach($wrp->result() as $r) {
				...
				*/
				...
				if ($r->num_state==3){
					$action .="<a href='".base_url()."verifikasi/dok/".$r->id_permohonan."' class='btn btn-info btn-sm' title='Verifikasi Dokumen' id=". $r->id_permohonan."><i class='fa fa-check-square-o'></i></a>&nbsp;";
					$action .="<a onclick='cancel(this.id)' class='btn btn-danger btn-sm' title='Tolak Permohonan' id=". $r->id_permohonan."><i class='fa fa-close'></i></a>&nbsp;";
					$kode="<span class='label label-success'>C</span>";
				}
				
				if ($r->num_state==4){
					$action .="<a href='".base_url()."verifikasi/dok/".$r->id_permohonan."' class='btn btn-success btn-sm' title='List Dokumen' id=". $r->id_permohonan."><i class='fa fa-list'></i></a>&nbsp;";
					$action .="<a href='".base_url()."verifikasi/dok2/".$r->id_permohonan."' class='btn btn-primary btn-sm' title='Validasi Dokumen' id=". $r->id_permohonan."><i class='fa fa-check'></i></a>";
					$kode="<span class='label label-primary'>V</span>";
				}
	...
	public function ajax_submitverif($id=0)
	{
		$nofill=$this->isidok->cekuncompletedok($id);
		//$nofill=0;
		if ($nofill){
			$data = array(
					'num_state' => 4,
			...
			echo json_encode(array("status" => FALSE, "info" => "GAGAL. Dokumen belum lengkap"));
		}
	}
	
	public function ajax_submitvalid($id=0)
	{		
		$data = array(
				'num_state' => 5,
			);
	...
	public function ajax_reject($id)
	{
		...
		if ($this->permohonan->update(array('id_permohonan' => $id), $data) > 0) {
			echo json_encode(array("status" => TRUE, "info" => "Reject sukses"));
		}else{
			echo json_encode(array("status" => FALSE, "info" => "Reject gagal"));
		}
