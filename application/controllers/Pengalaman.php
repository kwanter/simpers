...
class Pengalaman extends CI_Controller {
	...
	public function data($id=0)
	{
		...
		$this->pagenav('vw_pengalaman',$data);
	}
	...
	public function ajax_add($id=0)
	{
		...
		if ($this->pengalaman->save($data) > 0){
	...
	public function ajax_list($id=0)
	{
		  ...
		  $wrp = $this->pengalaman->getdata($id);
		  $data = array();
		  $a=1;
		  foreach($wrp->result() as $r) {
				...
				*/
				$action ="<a onclick='edit(this.id)' class='btn btn-info btn-xs' title='Edit' id=". $r->id_rwpengalaman."><i class='fa fa-pencil'></i></a>&nbsp;";
				$action .="<a onclick='deldata(this.id)' class='btn btn-danger btn-xs' title='Hapus' id=". $r->id_rwpengalaman."><i class='fa fa-trash'></i></a>&nbsp;";

				$t = new DateTime($r->tgl_mulai); 
				$tgl=$t->format('d M Y');
				$t2 = new DateTime($r->tgl_selesai); 
	...
	{
		$data = $this->pengalaman->getbyid($id);
	...
	private function _validate()
	{
		...
		if($this->input->post('nama') == '')
		{
			...
			$data['error_string'][] = 'Nama Tempat wajib diisi';
			$data['status'] = FALSE;
		}
	...
	public function ajax_update()
	{
		...
		if ($this->pengalaman->update(array('id_rwpengalaman' => $this->input->post('id')), $data)) {
